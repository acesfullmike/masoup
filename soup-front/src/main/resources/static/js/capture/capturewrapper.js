...
{
		alert("到此处下载: http://www.ggniu.cn/download/CaptureInstall.exe");
...
function StartCapture()
{
    ...
    //从返回值来解析显示  
    console.log("after capture: " + captureRet, emCaptureFailed, emCaptureUnknown);
    if(captureRet == emCaptureFailed || captureRet == emCaptureUnknown)
    {		
    		var url = "http://www.ggniu.cn/download/CaptureInstall.exe";
    		//url = "https://gitee.com/thewire/todos/raw/master/high/capture/CaptureInstall.exe";
    		//alert("请先安装截屏插件，然后刷新浏览器重试，地址：\n" + url);
    		if (confirm("点击 [确定] 下载插件，安装然后刷新浏览器重试。")) {
    				window.open(url);
				}
	}
}
